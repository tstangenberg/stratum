...
- [ ] CI runs `go run github.com/google/addlicense -check -l agpl -c "Thorben Stangenberg" -s only -ignore "**/*.gen.go" ./...` and fails if any file is missing a header
- [ ] The failure message tells the contributor which files are missing headers and what command to run to fix them
- [ ] Running `go run github.com/google/addlicense -l agpl -c "Thorben Stangenberg" -s only -ignore "**/*.gen.go" ./...` locally adds missing headers without modifying existing ones
