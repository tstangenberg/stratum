...
The `list` query is generated for every type. `limit` is provided by the core (default 100, hard max 1000). `offset` is provided by the `pagination-simple` plugin from the MVP bundle. See ADR-1008 (plugin architecture).
...
- [ ] `limit` exceeding the hard maximum (1000) returns a GraphQL error
